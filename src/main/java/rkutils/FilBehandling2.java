...
class FilBehandling2 {
	...
	public static void main(String args[]) throws IOException {
		...
		FilBehandling2 f = new FilBehandling2();
	...
	private class JoinByData {
		...
		JoinByData(int fil1Startpos, int antallTegn, int fil2Startpos,
				String fil1Label, String fil2Label) {
			this.fil1Startpos = fil1Startpos;
			this.antallTegn = antallTegn;
			this.fil2Startpos = fil2Startpos;
			this.fil1Label = fil1Label;
			this.fil2Label = fil2Label;
		}
	}
	...
	int antInnfilLinjer = 0, antUfilLinjer = 0;
	...
	private void bestemSdvFeltLengde() throws IOException {
		...
		try {
			...
			while (linje != null) {
				...
				// ¤%&
				String tekst = "";
				while ((tegnTeller < linjeLengde) && !(linjeTeller > 1 && feltTeller > sisteSdvFeltKolonneNummer)) {
				...
				if (feltFilTomInd & linjeTeller == 1) {
					...
					if (sisteSdvFeltKolonneNummer == 0) {
						sisteSdvFeltKolonneNummer = antUtFelt;
					} 
				}
		...
		for (Iterator itFeltliste = utfilFeltDefListe.iterator(); itFeltliste
			...
			if (ufd.filnummer != 0 & (ufd.antallTegn < sdvFeltLengdeTab[ufd.startpos]))
		...
		try {
			...
			while (linje != null) {
				...
				for (Iterator itFeltliste = utfilFeltDefListe.iterator(); itFeltliste
					...
					mld(mldInd, "  startpos: " + ufd.startpos + " antallTegn: "
							+ ufd.antallTegn + " feltTeller: " + feltTeller);
					...
					if (ufd.filnummer == 0)
						...
						if (ufd.arvFeltInd) {
							...
							if (linje.substring(startpos2tmp,
								...
								ufd.arvFeltVerdi = linje.substring(startpos,
										sluttpos);
								mld(mldInd, "arvfeltverdi: " + ufd.arvFeltVerdi);
							}
